...
{ 
	...
	if(p_Dlg->m_open)
	{
		try
		{
			...
			{
				if(!USBIO_MemWriteAddr0( p_Dlg->mIndex, mBuf, &mLen ) )
				{
					return(false);
				}
			}
			...
			{
				if(!USBIO_MemWriteAddr1( p_Dlg->mIndex, mBuf, &mLen ) )
				{
					return(false);
				}
			}
...
{   UCHAR mBuf[1]="";
	...
	if(p_Dlg->m_open)
	{
		if (m_add0==0)
		{
			...
			{
				MessageBox("MEM地址方式0读组合按钮失败！ ","USB2I2C",MB_OK|MB_ICONSTOP);
				return;
			}
		}
		else if(m_add0=1)
		{
			...
			{
				MessageBox("MEM地址方式1读组合按钮失败！ ","USB2I2C",MB_OK|MB_ICONSTOP);
				return;
			}
		}
		//按钮状态显示
...
if (MemWriteDate()==false)  //发LED状态
		MessageBox("发送LED状态值失败！","USB2I2C",MB_OK|MB_ICONSTOP);
