...
void CEppPage::OnButtonEppread0()	//EPP方式读数据
{
	...
	if(mLen <= 0)
	{
		MessageBox("请输入读取长度","USB2I2C",MB_OK);
		return;
	}
	if(p_Dlg->m_open)
	{
		try
		{
			if(!USBIO_EppReadData(p_Dlg->mIndex,  &mBuf[0],&mLen))
			{   m_epplen0.Format("%x", 0);
				MessageBox("EPP方式读数据失败！ ","USB2I2C",MB_OK|MB_ICONSTOP);
			}
			else
			{
	...
	{
		MessageBox("设备未打开！","USB2I2C",MB_OK|MB_ICONSTOP);
	}
	UpdateData(false);
}
...
void CEppPage::OnButtonEppwrite0()		//EPP方式写数据
{
	...
	{
		MessageBox("请输入写数据和长度！","USB2I2C",MB_OK);
		return;
	}	
	...
	if(p_Dlg->m_open)
	{
		if(!USBIO_EppWriteData( p_Dlg->mIndex, &buffer[0], &mLen ) )
		{   
			m_epplen0.Format("%x", 0);
		//	UpdateData(FALSE);
			MessageBox("EPP写数据块0失败！","USB2I2C",MB_OK|MB_ICONSTOP);
		}
		else
		{
...
void CEppPage::OnButtonEppread1()		//EPP方式读地址
{
	...
	if(mLen<=0)
	{
		MessageBox("请输入读数据长度","USB2I2C",MB_OK);
		return;
	}
	if(p_Dlg->m_open)
	{

		if(!USBIO_EppReadAddr(p_Dlg->mIndex, &mBuf[0], &mLen ))
		{   m_epplen1.Format("%x", 0);
			MessageBox("EPP方式读地址失败！ ","USB2I2C",MB_OK|MB_ICONSTOP);
		}
		else
		{
			CHAR buffer[mMAX_BUFFER_LENGTH * 3];
...
void CEppPage::OnButtonEppwrite1()		//EPP方式写地址
{
	...
	if(p_Dlg->m_open)
	{
		if(!USBIO_EppWriteAddr( p_Dlg->mIndex, &buffer[0], &mLen ) )
		{  
			m_epplen1.Format("%x", 0);
			MessageBox("EPP方式写地址失败！","USB2I2C",MB_OK|MB_ICONSTOP);
		}
		else
		{
			m_epplen1.Format("%x", mLen);
