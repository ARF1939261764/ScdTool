...
{   
	...
	if (mwlen==0 && mrlen==0)
	{
		MessageBox("请输入长度","USB2I2C",MB_OK);
		return;
	}
	
	else if(strlen(m_swrdatabuf) == 0 && (mwlen>0))
	{
		MessageBox("请输入要写入的数据","USB2I2C",MB_OK);
		return;
	}
	else if(mrlen < 0 )
	{
		MessageBox("请输入读数据的长度","USB2I2C",MB_OK);
		return;
	}	
	...
	if(p_Dlg->m_open)
	{
		try
		{
			if(!USBIO_StreamI2C (p_Dlg->mIndex, mwlen, &wbuffer[0], mrlen, &rbuffer[0] ))
			{   
				m_swrdatalen.Format("%x", 0);
				m_srddatalen.Format("%x", 0);     //返回已读写的数据个数
				MessageBox("流模式读写数据失败！","USB2I2C",MB_OK|MB_ICONSTOP);
			}
			else   //发送成功
			{
