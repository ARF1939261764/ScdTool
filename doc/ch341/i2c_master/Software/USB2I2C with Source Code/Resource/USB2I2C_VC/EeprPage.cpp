...
{ 
	...
	{
		MessageBox("请输入数据单元地址,长度","USB2I2C",MB_OK);
		return;
	}	
	...
	if(p_Dlg->m_open)
	{ 	if( !USBIO_WriteEEPROM( p_Dlg->mIndex,etype(m_eepsel), mDataAddr,mLen,buffer ) )
	{       mLen=0;
			MessageBox("读E2PROM数据失败！","USB2I2C",MB_OK|MB_ICONSTOP);
		}
...
{   
	...
	if(p_Dlg->m_open)
	{ 	if( !USBIO_ReadEEPROM( p_Dlg->mIndex,etype(m_eepsel), mDataAddr,mlen,mBuff ) )
...
_EEPROM_TYPE CEeprPage::etype(int EppSelId)  //根据单选按纽,选择EEP器件型号
{
