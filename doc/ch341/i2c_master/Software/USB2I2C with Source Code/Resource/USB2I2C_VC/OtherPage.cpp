...
void COtherPage::OnButtonI2cread()		//读I2C
{
	...
	{
		MessageBox("请输入数据单元地址","USB2I2C",MB_OK);
		return;
	}
	else if(strlen(m_devaddr) == 0)
	{
		MessageBox("请输入设备地址","USB2I2C",MB_OK);
		return;
	}
	if(strlen(m_dataaddr) > 2 || strlen(m_devaddr) > 2)
	{
		MessageBox("设备地址和数据单元地址都应不大于十六进制FFH","USB2I2C",MB_OK);
		return;
	}
	...
	if(p_Dlg->m_open)
	{
		...
		{
			MessageBox("I2C读数据失败！","USB2I2C",MB_OK|MB_ICONSTOP);
		}
		else
		{
			CHAR mtemp[4]="";		
...
void COtherPage::OnButtonI2cwrite()			//写I2C
{
	...
	{
		MessageBox("请输入设备地址、数据单元地址和数据","USB2I2C",MB_OK);
		return;
	}
	if(strlen(m_dataaddr) > 2 || strlen(m_devaddr) > 2 || strlen(m_data) > 2)
	{
		MessageBox("设备地址、数据单元地址和待写入数据都应不大于十六进制FFH","USB2I2C",MB_OK);
		return;
	}
	...
	if(p_Dlg->m_open)
	{
		...
		{
			MessageBox("I2C写数据失败！","USB2I2C",MB_OK|MB_ICONSTOP);
		}
		else
		{
			MessageBox("I2C写数据成功！","USB2I2C",MB_OK);			
		} 
	}
	else
	{
